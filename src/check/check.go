...
func (c *Checker) EvaluateConstantExpr(decl *tree.ConstDecl, expr tree.Expr) tree.Expr {
	...
	case *tree.LiteralExpr:
		...
		case *tree.LiteralRune:
			Assert(lit.Value[0] == '\'', "rune literal not single-quoted")
			Assert(lit.Value[len(lit.Value)-1] == '\'', "rune literal not single-quoted")
	...
	case *tree.BinaryExpr:
		...
		case *tree.ConstFloatExpr:
			return c.EvaluateBinaryOpFloat(expr.Op, c.EvaluateConstantFloatExpr(decl, left), right.Value)
		}

		switch left := left.(type) {
		case *tree.ConstIntExpr:
			return c.EvaluateBinaryOpInt(expr.Op, left.Value, c.EvaluateConstantIntExpr(decl, right))
		case *tree.ConstUintExpr:
			return c.EvaluateBinaryOpUint(expr.Op, left.Value, c.EvaluateConstantUintExpr(decl, right))
		case *tree.ConstFloatExpr:
			return c.EvaluateBinaryOpFloat(expr.Op, left.Value, c.EvaluateConstantFloatExpr(decl, right))
		case *tree.ConstBoolExpr:
			return c.EvaluateBinaryOpBool(expr.Op, left.Value, c.EvaluateConstantBoolExpr(decl, right))
		case *tree.ConstStringExpr:
			return c.EvaluateBinaryOpString(expr.Op, left.Value, c.EvaluateConstantStringExpr(decl, right))
		case *tree.ConstRuneExpr:
			return c.EvaluateBinaryOpRune(expr.Op, left.Value, c.EvaluateConstantRuneExpr(decl, right))
		default:
	...
	case *tree.UnaryExpr:
		...
		case *tree.ConstIntExpr:
			value := c.EvaluateUnaryOpInt(expr.Op, operand.Value)
			return &tree.ConstIntExpr{Value: value}
		case *tree.ConstUintExpr:
			value := c.EvaluateUnaryOpUint(expr.Op, operand.Value)
			return &tree.ConstUintExpr{Value: value}
		case *tree.ConstFloatExpr:
			value := c.EvaluateUnaryOpFloat(expr.Op, operand.Value)
			return &tree.ConstFloatExpr{Value: value}
		case *tree.ConstBoolExpr:
			return c.EvaluateUnaryOpBool(expr.Op, operand.Value)
		case *tree.ConstStringExpr:
			return c.EvaluateUnaryOpString(expr.Op, operand.Value)
		default:
			panic("unreachable")
		}
	case *tree.CallExpr:
...
func (c *Checker) EvaluateBinaryOpInt(op tree.BinaryOp, left int64, right int64) tree.Expr {
...
func (c *Checker) EvaluateBinaryOpUint(op tree.BinaryOp, left, right uint64) tree.Expr {
	if v, err := EvaluateBinaryOpNumericCompare(op, left, right); err == nil {
		return &tree.ConstBoolExpr{Value: v}
	}
	if v, err := EvaluateBinaryOpIntegerArithmetic(op, left, right); err == nil {
...
func (c *Checker) EvaluateBinaryOpFloat(op tree.BinaryOp, left, right float64) tree.Expr {
	if v, err := EvaluateBinaryOpNumericCompare(op, left, right); err == nil {
		return &tree.ConstBoolExpr{Value: v}
	}
...
func (c *Checker) EvaluateBinaryOpRune(op tree.BinaryOp, left rune, right rune) tree.Expr {
	if v, err := EvaluateBinaryOpNumericCompare(op, left, right); err == nil {
		return &tree.ConstBoolExpr{Value: v}
	}
	if v, err := EvaluateBinaryOpIntegerArithmetic(op, left, right); err == nil {
...
func (c *Checker) EvaluateBinaryOpBool(op tree.BinaryOp, left, right bool) tree.Expr {
	switch op {
...
func (c *Checker) EvaluateBinaryOpString(op tree.BinaryOp, left, right string) tree.Expr {
	switch op {
	case tree.BinaryOpAdd:
...
func (c *Checker) EvaluateUnaryOpInt(op tree.UnaryOp, value int64) int64 {
...
func (c *Checker) EvaluateUnaryOpFloat(op tree.UnaryOp, value float64) float64 {
	switch op {
...
func (c *Checker) EvaluateUnaryOpBool(op tree.UnaryOp, value bool) tree.Expr {
	switch op {
	case tree.UnaryOpNot:
		return &tree.ConstBoolExpr{Value: !value}
	default:
		panic(fmt.Sprintf("unsupported bool unary op %v", op))
	}
}

func (c *Checker) EvaluateUnaryOpString(op tree.UnaryOp, value string) tree.Expr {
